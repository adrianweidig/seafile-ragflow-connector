...
wie `0.1.12` gepinnt werden; `latest` ist eine Komfortoption für Smoke-Tests und
frische Testumgebungen.

```bash
docker pull ghcr.io/adrianweidig/seafile-ragflow-connector:0.1.12
```
...
```bash
docker save \
  ghcr.io/adrianweidig/seafile-ragflow-connector:0.1.12 \
  postgres:16 \
  redis:7 \
...
```env
CONNECTOR_IMAGE=seafile-ragflow-connector:0.1.12
