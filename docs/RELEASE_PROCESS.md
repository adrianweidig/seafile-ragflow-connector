...
Dieses Repository enthält aktuell Paketmetadaten mit Version `2.6.3`, einen
...
Für 2.6.3 gehört zur Release-Abnahme außerdem ein Browser-Smoke gegen das im
...
`seafile-ragflow-connector-airgap-2.6.3-<sha7>.7z`. Es enthält mindestens:
...
docker-images/
  ...
  seafile-ragflow-connector_2.6.3_linux-amd64.docker-image.tar
...
install/
  ...
  docs/
python-dist/
  seafile_ragflow_connector-2.6.3-py3-none-any.whl
  seafile_ragflow_connector-2.6.3.tar.gz
repo/
  seafile-ragflow-connector-2.6.3.git.bundle
  seafile-ragflow-connector-source-2.6.3.zip
```

Git-Bundle und Source-ZIP müssen exakt den veröffentlichten Release-Commit
abbilden; die Connector-Image-Tar muss zum verifizierten `2.6.3`-Digest gehören.
...
```powershell
$sha7 = (git rev-parse --short=7 'v2.6.3^{commit}').Trim()
$bundlePath = "F:\seafile-ragflow-connector-airgap-2.6.3-$sha7.7z"
